...
class KnowledgeBase:
    ...
    def resolution_refutation(self, goal, max_steps=100):
        ...
        log   = [f"Goal: {goal}", f"Added: [{neg_goal}]"]
        steps = 0
        changed = True
            ...
            for i in range(n):
                for j in range(i + 1, n):
                    ...
                    for lit in ci:
                        ...
                        steps += 1
                        if len(resolvent) == 0:
                            log.append(f"Step {steps}: contradiction found!")
                        ...
                        if key not in self.clause_set:
                            log.append(f"Step {steps}: {resolvent}")
                            self.clause_set.add(key)
                            clauses.append(resolvent)
                            changed = True
                        break
        log.append(f"Cannot prove: {goal}")
...
class Game:
    ...
    def _visit_cell(self, r, c):
        ...
        breeze, stench = get_percepts(r, c, self.pits, self.wumpus, self.rows, self.cols)
        self.percepts = []
        ...
        self.agent_log.append(f"At ({r},{c}) | Percepts: {self.percepts or ['None']}")
        ...
        for nr, nc in nb:
            ...
            if pit_ok and w_ok:
                ...
                pit_d, _, _ = self.kb.resolution_refutation(f"pit_{nr}_{nc}")
                w_d,   _, _ = self.kb.resolution_refutation(f"wumpus_{nr}_{nc}")
                if pit_d or w_d:
    ...
    def to_dict(self):
        ...
        return {
            ...
            "alive": self.alive, "won": self.won, "stuck": self.stuck,
...
# Store game in memory (Vercel is stateless so each session is fresh)
game_store = {}
...
.controls{display:flex;flex-wrap:wrap;gap:10px;align-items:flex-end;background:#161b22;border:1px solid #30363d;border-radius:8px;padding:14px;margin-bottom:16px}
.ctrl label{display:block;font-size:.62rem;color:#6e7681;margin-bottom:3px;text-transform:uppercase}
.ctrl input[type=number]{width:65px;background:#0d1117;border:1px solid #30363d;color:#58a6ff;font-family:inherit;font-size:.95rem;padding:5px 8px;border-radius:5px;outline:none}
...
.btn{padding:7px 18px;border:none;border-radius:5px;font-family:inherit;font-size:.8rem;font-weight:bold;cursor:pointer}
...
.status{text-align:center;padding:8px 14px;border-radius:6px;font-size:.78rem;margin-bottom:10px;background:#0d1117;border:1px solid #30363d;color:#58a6ff}
...
.cell{aspect-ratio:1;border-radius:6px;border:1px solid #30363d;min-height:50px;display:flex;flex-direction:column;align-items:center;justify-content:center;position:relative;transition:background .25s;overflow:hidden}
...
.card h3{font-size:.6rem;text-transform:uppercase;letter-spacing:.1em;color:#6e7681;margin-bottom:10px;padding-bottom:6px;border-bottom:1px solid #30363d}
...
.log-box{background:#0d1117;border:1px solid #30363d;border-radius:5px;padding:8px;height:150px;overflow-y:auto;font-size:.6rem;line-height:1.9}
.l-safe{color:#3fb950}.l-warn{color:#d29922}.l-dead{color:#f85149}.l-info{color:#58a6ff}.l-plain{color:#6e7681}
.proof-box{background:#0d1117;border:1px solid #30363d;border-radius:5px;padding:8px;height:130px;overflow-y:auto;font-size:.57rem;line-height:1.9;color:#6e7681}
.proved{color:#3fb950;font-weight:bold}.failed{color:#d29922}.pstep{color:#c9d1d9}.psep{color:#30363d}
</style>
</head>
<body>
<h1>Wumpus World &mdash; Logic Agent</h1>
<p class="sub">Knowledge-Based Agent &bull; Propositional Logic &bull; Resolution Refutation</p>
<div class="controls">
...
<script>
let revealed=false,autoTimer=null;
async function api(path,data={}){
  const res=await fetch(path,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)});
  return res.json();
}
async function newGame(){
  stopAuto();revealed=false;
  document.getElementById('btn-reveal').textContent='Reveal All';
  const s=await api('/api/new',{rows:+document.getElementById('in-rows').value||4,cols:+document.getElementById('in-cols').value||4,pits:+document.getElementById('in-pits').value||2});
  render(s);
...
async function doStep(){
  const s=await api('/api/step');render(s);
  if(!s.alive||s.won||s.stuck){stopAuto();document.getElementById('btn-step').disabled=true;document.getElementById('btn-auto').disabled=true;}
}
function render(s){renderGrid(s);renderMetrics(s);renderPercepts(s);renderStatus(s);renderLog(s);renderProof(s);}
function renderGrid(s){
  const gc=document.getElementById('grid');
  gc.style.gridTemplateColumns=`repeat(${s.cols},1fr)`;gc.innerHTML='';
  const cells=[...s.grid].sort((a,b)=>b.r-a.r||a.c-b.c);
  for(const cell of cells){
    const div=document.createElement('div');div.className='cell';
    if(cell.agent)div.classList.add('c-agent');
    else if(revealed&&(cell.pit||cell.wumpus))div.classList.add('c-danger');
    else if(cell.danger)div.classList.add('c-danger');
    else if(cell.visited)div.classList.add('c-visited');
    else if(cell.safe)div.classList.add('c-safe');
    else div.classList.add('c-unknown');
    const co=document.createElement('div');co.className='coord';co.textContent=`${cell.r},${cell.c}`;div.appendChild(co);
    let icon='',lbl='';
    ...
    if(icon){const ic=document.createElement('div');ic.className='icon';ic.textContent=icon;div.appendChild(ic);}
    if(lbl){const lb=document.createElement('div');lb.className='lbl';lb.textContent=lbl;div.appendChild(lb);}
    if(cell.visited&&!cell.agent){const p=(cell.breeze?'B':'')+(cell.stench?'S':'');if(p){const pd=document.createElement('div');pd.className='perc';pd.textContent=p;div.appendChild(pd);}}
    gc.appendChild(div);
  }
}
function renderMetrics(s){document.getElementById('m-steps').textContent=s.steps;document.getElementById('m-infer').textContent=s.inference_steps;document.getElementById('m-safe').textContent=s.safe_count;document.getElementById('m-kb').textContent=s.kb_facts;}
function renderPercepts(s){
  const pl=document.getElementById('percepts');pl.innerHTML='';
  if(!s.percepts.length){pl.innerHTML='<span class="tag none">None - All Clear</span>';return;}
  for(const p of s.percepts){const sp=document.createElement('span');sp.className='tag '+p.toLowerCase();sp.textContent=p;pl.appendChild(sp);}
}
...
function startAuto(){
  ...
  async function tick(){await doStep();if(!document.getElementById('btn-step').disabled)autoTimer=setTimeout(tick,delay());}
  autoTimer=setTimeout(tick,delay());
}
function stopAuto(){if(autoTimer){clearTimeout(autoTimer);autoTimer=null;}document.getElementById('btn-auto').textContent='Auto Run';}
...
document.getElementById('btn-reveal').addEventListener('click',async()=>{
  ...
  renderGrid(await api('/api/state'));
});
document.getElementById('speed').addEventListener('input',function(){document.getElementById('speed-lbl').textContent=this.value+'ms';});
</script>
</body>
</html>"""

# Global game storage (per session via simple dict)
games = {}
...
def new_game():
    ...
    games['current'] = Game(rows, cols, pits)
    return jsonify(games['current'].to_dict())
